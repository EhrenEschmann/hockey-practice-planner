...
A practice moves through **stages** in practice details (**+ Practice → Sharing**): *Draft* → **Send to coaches →** → **Release to team →**, and back again with **← Pull back** (access ends right away). Nothing is visible to anyone until you press the button. **Who** gets it comes from the **👥 Team** roster — coaches' emails, players' family contacts — plus any extra emails on the practice; add someone to the roster once and they get every practice released from then on; remove them and they lose access (and their offline copy). Links carry no secret: `/coach/<id>` and `/team/<id>` only open for people the stage and the roster allow. Old `#view=…` / `#team=…` links still work and redirect. Someone who opens a practice that isn't released to them yet sees "isn't available yet"; the top-left 🏒 goes back to their list. Under the top bar every practice says which one it is — **team · weekday, date · start time** — with **‹ ›** to step to that team's earlier and later practices (only the ones released to that person; tapping the label opens the full list). If it is not the practice the calendar points at — the next one (today's counts all day), else the most recent — a **red banner** says so ("⚠ This is an older practice (Mon, Sep 14). The next practice is Wed, Sep 23 · 5:00pm — tap to open it.") so nobody runs last week's plan from an old link.
