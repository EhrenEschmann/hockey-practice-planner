...
A practice moves through **stages** in practice details (**+ Practice → Sharing**): *Draft* → **Send to coaches →** → **Release to team →**, and back again with **← Pull back** (access ends right away). Nothing is visible to anyone until you press the button. **Who** gets it comes from the **👥 Team** roster — coaches' emails, players' family contacts — plus any extra emails on the practice; add someone to the roster once and they get every practice released from then on; remove them and they lose access (and their offline copy). Links carry no secret: `/coach/<id>` and `/team/<id>` only open for people the stage and the roster allow. Old `#view=…` / `#team=…` links still work and redirect. Someone who opens a practice that isn't released to them yet sees "isn't available yet"; the top-left 🏒 goes back to their list.
